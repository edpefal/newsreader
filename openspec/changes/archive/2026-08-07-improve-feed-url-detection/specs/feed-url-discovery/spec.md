...
El sistema SHALL mostrar un mensaje de error genérico y único cuando ningún candidato (la URL original ni ninguno de los candidatos heurísticos aplicables) resulte ser un feed válido, indicando al usuario que pegue la URL exacta del feed RSS.

#### Scenario: Ningún candidato resulta válido
- **WHEN** el usuario ingresa una URL que no es un feed válido y ninguno de los candidatos heurísticos aplicables (inserción de path ni sufijos genéricos) resulta serlo tampoco
