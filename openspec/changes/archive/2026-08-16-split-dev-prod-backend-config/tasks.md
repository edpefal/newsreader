...
- [x] 4.3 `flutter run` (sin flags) confirmado en modo "dev", contra el proyecto dev — fuente agregada y sincronizada correctamente en `reevo-dev` (0→1, prod sin cambios), navegación funcionando. Encontró y arregló 2 bugs reales de paso (no relacionados a dev/prod en sí): Hive compartía caché local entre ambientes (fix: nombres de box por ambiente en `AppConstants`), e Inbox no se refrescaba al agregar fuente/cambiar de pestaña (2 fixes en `inbox_screen.dart`/`router.dart`). "Generar un resumen" de punta a punta confirmado 2026-08-16 contra dev, sin afectar prod.
