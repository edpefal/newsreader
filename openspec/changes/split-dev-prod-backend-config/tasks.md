...
- [ ] 4.3 `flutter run` (sin flags) y confirmar en el log que arranca en modo "dev", contra el proyecto dev — probar agregar una fuente y generar un resumen sin que afecte al proyecto prod. **Parcialmente verificado**: `Ambiente: dev` confirmado en log, fuente agregada y sincronizada correctamente en `reevo-dev` (0→1, prod se confirmó sin cambios en 96), navegación funcionando. Encontró y arregló 2 bugs reales de paso (no relacionados a dev/prod en sí): Hive compartía caché local entre ambientes (fix: nombres de box por ambiente en `AppConstants`), e Inbox no se refrescaba al agregar fuente/cambiar de pestaña (2 fixes en `inbox_screen.dart`/`router.dart`). **Falta**: probar "generar un resumen" de punta a punta — bloqueado por la infraestructura de Superwall (ver `gate-daily-summary-behind-superwall-paywall/tasks.md` sección 8, en curso).
